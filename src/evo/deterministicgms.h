...
public:
    ...
    {
        return gmUniquePropertyMap.count(::SerializeHash(v, SER_GETHASH, PROTOCOL_VERSION | ADDRV2_FORMAT)) != 0;
    }
    template <typename T>
    CDeterministicGMCPtr GetUniquePropertyGM(const T& v) const
    {
        auto p = gmUniquePropertyMap.find(::SerializeHash(v, SER_GETHASH, PROTOCOL_VERSION | ADDRV2_FORMAT));
        if (!p) {
            return nullptr;
        }
