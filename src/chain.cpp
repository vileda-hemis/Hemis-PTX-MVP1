...
const CBlockIndex* CBlockIndex::GetAncestor(int height) const
{
    ...
    while (heightWalk > height) {
        ...
        // BUG-040: tolerate an unbuilt pskip — degrade to the pprev walk (port of
        // Bitcoin PR #5927; PIVX never took it, which left the skip branch unguarded).
...
int64_t CBlockIndex::MinPastBlockTime() const
{
    ...
    // so we allow the nBlockTimeProtocolV2 (Hemis v4.0) to be at most (180-14) seconds earlier than previous block
