...
**Next-free: KDD-081 / ODC-056 (unchanged — this entry consumes BUG-023/024/025).**
...
| BUG-023 | 2026-07-29 | ★ VERIFYDB STARTUP CLOBBER — `CVerifyDB` level-3 disconnect applies REAL in-memory special-tx undo (`GetLotteryState() = prevState`, specialtx_validation.cpp:1495) outside both sandboxes (throwaway coins cache; evoDb rollback tx); level 3 never reconnects, load-then-verify order, nothing reloads: any restart with a PTXCOALESCE within `-checkblocks` (6) of tip silently regresses the node's live lottery state → h720 TRIPLE-SEALED boundary (missingorspent / P11 missing-at-boundary / P2 wrong-input), fleet-wide, image-independent; `verifychain` RPC reaches the same code. Proof: correct-load-line vs pool-14.00 split on every node, every restart. Restartless repair (invalidate/reconsider) demonstrated, then chain declared disposable. **See full entry above.** PTX. | fix owed (this entry) | Root-caused 2026-07-29; full entry + row 2026-07-30 |
| BUG-024 | 2026-07-29 | ★ fJustCheck COALESCE→PAYOUT ORDERING ASYMMETRY — coalesce-apply is fJustCheck-gated while payout P2/P5 read the global regardless, so a coalesce+payout block (what the assembler builds at any settlement boundary with pending PTXSESS) fails its own TestBlockValidity `ptxpayout-wrong-input` though it would connect: valid-on-connect, unbuildable-by-producer → structural liveness wedge the first time boundary × roll demand coincide (all four prior settlements were payout-only; h720 was the first coincidence; reproduced post-state-repair at 22:25Z with the mempool.dat-persisted PTXSESS `a5fd8cc3…` in every mempool). Probable 04:05 onset (pre-812c606 deaths logged no reason — recorded at inference confidence). **See full entry above.** PTX. | fix owed (this entry) | Root-caused 2026-07-29; full entry + row 2026-07-30 |
| BUG-025 | 2026-07-29 | ★ `ptx_pose.dat` SURVIVES `-reindex` — node-local wall-clock-accumulated PoSe state feeds P10/P11 winner selection, loads unconditionally at init.cpp:1448, not wiped by -reindex (evoDb-only wipe): gm01's replay validated committed h480 against 88 FUTURE pose records → `ptxpayout-wrong-recipient` fork at 479. ★ 2×2 counterfactual (datadir × image): clean replays accept 0→719 under BOTH images; both BUG-022-labelled commits are producer-only — **`225d127` EXONERATED; no retro-rejecting rule ever existed**. The class: consensus inputs not derived purely from the chain. Wipe-on-reindex owed minimum; pose-as-chain-function is the design question. **See full entry above.** PTX. | fix owed (this entry) | Root-caused + exoneration 2026-07-29→30; full entry + row 2026-07-30 |
