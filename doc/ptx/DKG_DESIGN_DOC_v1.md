...
| BUG-015 | 2026-06-01 | Section B gate on coinstake vout count — burn-split diagnostic. Default wallet config never triggers the split path (gm01 UTXOs above `stakeSplitThreshold`), so burn always fired across all 3 Section B samples. PTX (lottery/roll). ★ **Owed task still open:** set `stakeSplitThreshold` below gm01 UTXOs, force a `stakerOuts>2` block, confirm burn absent. | a86ee16 | Landed (diagnostic + gate); owed task OPEN; back-filled 2026-07-24 |
