#!/usr/bin/env python3
"""
Hemis PTX Phase 2 — Live Node Test Suite v7.1
==============================================
199 tests across 17 categories.

v7.1 changes over v7:
...
def t181():
    """3 concurrent sessions each with 512-item exclude — concurrency stress.
    Validates that the node handles concurrent exclude-heavy sessions without
    corrupting state, crashing, or producing exclusion violations.
    """
    errors, violations = [], []
    lock = threading.Lock()

    def worker(idx):
        excl = list(range(1, 513))
        r, e = roll(1, 1, 1000, False, exclude=excl,
                    game_id=f"conc_excl_{idx}", salt=mksalt("conc_excl", idx))
        with lock:
            if e:
                errors.append(f"worker {idx}: {str(e)[:50]}")
            elif r:
                v = r["results"][0]
                if v in excl:
                    violations.append(f"worker {idx}: exclusion violation result={v}")
            else:
                errors.append(f"worker {idx}: nil response")
    ...
    alive = node_alive()
    if not alive:
        return False, "NODE CRASHED during concurrent 512-item exclude sessions"
    if violations:
        return False, f"Exclusion violations in concurrent sessions: {violations}"
    # Errors are acceptable if BUG-011 related (node accepted or returned error)
    if errors:
        return True, f"Concurrent sessions completed with errors (BUG-011 related): {errors}"
    return True, "3 concurrent 512-item exclude sessions — no crashes, no violations"


def t182():
    """Latency at 100/300/512 exclude items under 2 concurrent sessions.
    Compares against T177 single-session baseline to detect concurrency overhead.
    Node must not time out (TIMEOUT=45s). Results logged for ODC-015 hardware table.
    """
    latencies = []
    for size in [100, 300, 512]:
        excl = list(range(1, size + 1))
        # Two concurrent calls at the same size
        results_lat = []
        lock = threading.Lock()
        def worker_lat(sz=size, ex=excl):
            t0 = time.time()
            r, e = roll(1, 1, sz + 500, False, exclude=ex,
                        game_id="excl_conc_lat", salt=mksalt("conc_lat", sz, time.time_ns()))
            elapsed = time.time() - t0
            with lock:
                results_lat.append((elapsed, bool(r), str(e)[:30] if e else ""))
        threads = [threading.Thread(target=worker_lat) for _ in range(2)]
        for t in threads: t.start()
        for t in threads: t.join(timeout=90)
        alive = node_alive()
        if not alive:
            return False, f"NODE CRASHED during concurrent latency test at excl={size}"
        avg = sum(x[0] for x in results_lat) / max(len(results_lat), 1)
        latencies.append(f"excl={size}: avg={avg:.3f}s over {len(results_lat)} workers")
        print(f"         {latencies[-1]}")
    return True, " | ".join(latencies)
...
def main():
    ...
    print("═"*68)
    print(f"  HEMIS PTX PHASE 2 — LIVE NODE TEST SUITE v7.1  (199 tests)")
    print(f"  RPC:  {RPC_URL}")
    ...
    if not args.skip_excl_limit:
        ...
        for tid, name, fn in [
            ...
            ("T181","3 concurrent 512-item sessions — no crash, no violations",t181),
            ("T182","Exclude latency under 2 concurrent sessions at 100/300/512",t182),
