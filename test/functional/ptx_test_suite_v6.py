#!/usr/bin/env python3
"""
Hemis PTX Phase 2 — Live Node Test Suite v6
============================================
175 tests across 14 categories.

v6 additions over v5 (T151-T175) — Gap Coverage:
  EXCL-EXT  T151-T160  exclude edge cases not covered by T86-T89/T101-T120:
                        - confirmed tx_id chaining (real on-chain tx)
                        - mixed tx_id + integers at scale (400 items)
                        - 512-item exclude (MAX_EXCLUDE_COUNT boundary — must pass)
                        - 513-item exclude (one over limit — must return error 1016)
                        - duplicate values in exclude list
                        - out-of-range values in exclude list
                        - exhaustion via exclusion (pool reduced below count)
                        - count > pool via exclude at runtime (unique=True)
                        - exclude reduces pool to exactly count (tight fit)
                        - exclude all but one (single possible outcome)
  DEV/ERR   T161-T165  dev_seed (regtest-only mode) and error code verification:
                        - dev_seed returns deterministic result
                        - dev_seed same seed same params same result
                        - dev_seed different seed different result
                        - error code 1016 on exclude > MAX_EXCLUDE_COUNT
                        - error envelope format (code, message fields present)
  GAME-EXT  T166-T170  game_id and salt edge cases:
                        - game_id at 128 chars (max length)
                        - game_id with special characters
                        - same game_id different sessions → different seeds
                        - salt all-zeros (valid hex, edge value)
                        - salt all-f (valid hex, edge value)
  PREV-RND  T171-T175  prev_round_txid chaining errors:
                        - error 1008 prev_round_txid not found on chain
                        - error 1007 prev_round_txid unconfirmed (mempool)
                        - error 1009 prev_round_txid session mismatch
                        - valid prev_round_txid chains correctly
                        - multi-round chain: round 1 → round 2 via prev_round_txid

Usage (from node1 host, or inside a container with python3):
  python3 ptx_test_suite_v6.py --fast --skip-excl-probe --skip-lottery
  python3 ptx_test_suite_v6.py --skip-excl-probe --skip-lottery   # full stats
  python3 ptx_test_suite_v6.py                                     # everything
...
def _excl_probe(excl_size, iterations, pool_size, label):
    ...
    for i in range(iterations):
        r, e = roll(1, low, high, False, exclude=exclude,
                    game_id=f"{label}_{i}", salt=f"{i:08x}")
        if e:
...
def t14():
    r, e = roll(1, 1, 100, False, game_id="beacon_fmt", salt="ee04")
    if e: return None, f"RPC: {e}"
    b = r.get("beacon") or r.get("round_beacon","")
    return ok(isinstance(b,str) and len(b)==64 and all(c in "0123456789abcdef" for c in b), f"'{b}'")

def t15():
    r, e = roll(1, 1, 100, False, game_id="bh_field", salt="ee05")
    if e: return None, f"RPC: {e}"
...
def t16():
    seeds = set()
    for i in range(3):
        r, e = roll(1, 1, 100, False, game_id=f"seed_uniq_{i}", salt=f"eu{i:04x}")
        if e: return None, f"RPC: {e}"
...
def t17():
    beacons = set()
    for i in range(3):
        r, e = roll(1, 1, 100, False, game_id=f"beacon_uniq_{i}", salt=f"bu{i:04x}")
        if e: return None, f"RPC: {e}"
        b = r.get("beacon") or r.get("round_beacon","")
        beacons.add(b)
...
def t20():
    sigs = set()
    for i in range(3):
        r, e = roll(1, 1, 100, False, game_id=f"sig_uniq_{i}", salt=f"su{i:04x}")
        if e: return None, f"RPC: {e}"
...
def t21():
    r, e = roll(1, 1, 100, False, game_id="state_chk", salt="rs0001")
    if e: return None, f"RPC: {e}"
...
def t22():
    r, e = roll(1, 1, 100, False, game_id="committed_chk", salt="rs0002")
    if e: return None, f"RPC: {e}"
    rd = find_round_by_height(r["block_height"])
    if not rd: return None, "round not found (BUG-005)"
...
def t23():
    r, e = roll(1, 1, 100, False, game_id="withheld_chk", salt="rs0003")
    if e: return None, f"RPC: {e}"
    withheld = r.get("withheld", [])
    return ok(withheld == [], f"withheld non-empty: {withheld}")

def t24():
    r, e = roll(1, 1, 100, False, game_id="abstained_chk", salt="rs0004")
    if e: return None, f"RPC: {e}"
    abstained = r.get("abstained", [])
    return ok(abstained == [], f"abstained non-empty: {abstained}")

def t25():
    r, e = roll(1, 1, 100, False, game_id="round_id_chk", salt="rs0005")
    if e: return None, f"RPC: {e}"
    rd = find_round_by_height(r["block_height"])
    if not rd: return None, "round not found (BUG-005)"
    rid = rd.get("round_id","")
...
def t28():
    r1, e = roll(1,1,100,False,game_id="rid_uniq_a",salt="ru0001")
    if e: return None, f"RPC: {e}"
    r2, e = roll(1,1,100,False,game_id="rid_uniq_b",salt="ru0002")
    if e: return None, f"RPC: {e}"
    id1 = r1.get("round_id","")
    id2 = r2.get("round_id","")
...
def t29():
    r, e = roll(1, 0, 1, False, game_id="coin_flip", salt="gg01")
    if e: return None, f"RPC: {e}"
    return ok(r["results"][0] in [0,1], f"got {r['results'][0]}")

def t30():
    r, e = roll(1, 1, 6, False, game_id="d6", salt="gg02")
    if e: return None, f"RPC: {e}"
    return ok(1 <= r["results"][0] <= 6, f"got {r['results'][0]}")

def t31():
    r, e = roll(1, 1, 20, False, game_id="d20", salt="gg03")
    if e: return None, f"RPC: {e}"
    return ok(1 <= r["results"][0] <= 20, f"got {r['results'][0]}")

def t32():
    r, e = roll(1, 1, 100, False, game_id="d100", salt="gg04")
    if e: return None, f"RPC: {e}"
    return ok(1 <= r["results"][0] <= 100, f"got {r['results'][0]}")

def t33():
    r, e = roll(5, 1, 52, True, game_id="cards", salt="gg05")
    if e: return None, f"RPC: {e}"
    v = r["results"]
...
def t34():
    r, e = roll(52, 1, 52, True, game_id="full_deck", salt="gg06")
    if e: return None, f"RPC: {e}"
    return ok(sorted(r["results"])==list(range(1,53)), "not a full 52-card permutation")

def t35():
    r, e = roll(1, 1, 10000, False, game_id="raffle", salt="gg07")
    if e: return None, f"RPC: {e}"
    return ok(1 <= r["results"][0] <= 10000, f"got {r['results'][0]}")

def t36():
    r, e = roll(16, 1, 128, True, game_id="bracket", salt="gg08")
    if e: return None, f"RPC: {e}"
    v = r["results"]
...
def t37():
    r1, e = roll(5, 1, 100, True, game_id="seq_a", salt="gg09")
    if e: return None, f"RPC: {e}"
    r2, e = roll(5, 1, 100, True, game_id="seq_b", salt="gg10")
    if e: return None, f"RPC: {e}"
    return ok(r1["results"] != r2["results"], f"identical: {r1['results']}")

def t38():
    r1, e = roll(5, 1, 52, True, game_id="hand1", salt="gg11")
    if e: return None, f"RPC: {e}"
    hand1 = r1["results"]
    r2, e = roll(5, 1, 52, True, exclude=hand1, game_id="hand2", salt="gg12")
    if e: return None, f"RPC: {e}"
...
def t39():
    counts = [0, 0]
    for i in range(40):
        r, e = roll(5, 0, 1, False, game_id="stat_coin", salt=f"sc{i:04x}")
        if e: return None, f"RPC: {e}"
...
def t40():
    counts = [0]*7
    for i in range(100):
        r, e = roll(6, 1, 6, False, game_id="stat_d6", salt=f"sd{i:04x}")
        if e: return None, f"RPC: {e}"
        for v in r["results"]: counts[v] += 1
...
def t41():
    counts = [0]*21
    for i in range(50):
        r, e = roll(20, 1, 20, False, game_id="stat_d20", salt=f"s20{i:03x}")
        if e: return None, f"RPC: {e}"
        for v in r["results"]: counts[v] += 1
...
def t42():
    counts = [0]*101
    for i in range(20):
        r, e = roll(10, 1, 100, False, game_id="stat_d100", salt=f"s100{i:02x}")
        if e: return None, f"RPC: {e}"
        for v in r["results"]: counts[v] += 1
...
def t43():
    errors = []
    for i in range(20):
        r, e = roll(1, 1, 1000, False, game_id=f"seq_{i}", salt=f"sq{i:04x}")
        if e: errors.append(f"{i}: {e}")
    return ok(not errors, f"errors: {errors}")

def t44():
    r, e = roll(50, 1, 100, True, game_id="large_count", salt="lc0001")
    if e: return None, f"RPC: {e}"
    v = r["results"]
...
def t45():
    r, e = roll(1, 1, 2147483647, False, game_id="maxrange", salt="mr0001")
    if e: return None, f"RPC: {e}"
...
def t46():
    errors = []
    for i in range(15):
        r, e = roll(3, 1, 100, True, game_id=f"burst_{i}", salt=f"br{i:04x}")
...
def t78():
    ...
    for i in range(3):
        roll(1, 1, 100, False, game_id=f"stable_{i}", salt=f"st{i:04x}")
    pm2 = get_pose_map()
...
def t81():
    ...
    def do_roll(idx):
        r, e = roll(1, 1, 1000, False, game_id=f"concurrent_{idx}", salt=f"co{idx:04x}")
...
def t82():
    ...
    def do_roll(idx):
        r, e = roll(1, 1, 100, False, game_id=f"conc_seed_{idx}", salt=f"cs{idx:04x}")
        results_list[idx] = r; errors_list[idx] = e
...
def t83():
    ...
    def do_roll(idx):
        r, e = roll(1, 1, 100, False, game_id="same_game", salt=f"sg{idx:04x}")
        results_list[idx] = r; errors_list[idx] = e
...
def t84():
    errors = []
    for i in range(30):
        r, e = roll(1, 1, 100, False, game_id=f"load_{i}", salt=f"ld{i:04x}")
...
def t85():
    for i in range(30):
        roll(1, 1, 100, False, game_id=f"pre_load_{i}", salt=f"pl{i:04x}")
    return ok(node_alive(), "node unresponsive after 30-roll load")

def t86():
    fake_txid = "a" * 64
    r, e = roll(1, 1, 100, False, exclude=[fake_txid], game_id="txid_fake", salt="tx0001")
    alive = node_alive()
...
def t87():
    r1, e = roll(1, 1, 52, True, game_id="txid_source", salt="tx0002")
    if e: return None, f"first roll failed: {e}"
    pending_txid = r1.get("tx_id","b"*64) or "b"*64
    r2, e2 = roll(1, 1, 52, False, exclude=[pending_txid], game_id="txid_pending", salt="tx0003")
    alive = node_alive()
...
def t88():
    fake_txid = "c" * 64
    r, e = roll(1, 1, 10, False, exclude=[1,2,3,fake_txid,4,5], game_id="txid_mixed", salt="tx0004")
    alive = node_alive()
...
def t89():
    ...
    r, e = roll(1, 1, 100, False, exclude=fake_txids, game_id="txid_multi", salt="tx0005")
    alive = node_alive()
...
def t90():
    r1, e = roll(1, 1, 100, False, game_id="anchor_same", salt="bh0001")
    if e: return None, f"RPC: {e}"
    h1 = r1["block_height"]
    r2, e = roll(1, 1, 100, False, game_id="anchor_same", salt="bh0001")
    if e: return None, f"RPC: {e}"
    h2 = r2["block_height"]
...
def t91():
    r1, e = roll(1, 1, 100, False, game_id="anchor_diff", salt="bh0002")
    if e: return None, f"RPC: {e}"
    h1 = r1["block_height"]
    for _ in range(30):
        time.sleep(3)
        if blockcount() > h1: break
    r2, e = roll(1, 1, 100, False, game_id="anchor_diff", salt="bh0002")
    if e: return None, f"RPC: {e}"
    h2 = r2["block_height"]
...
def t92():
    r, e = roll(1, 1, 100, False, game_id="anchor_field", salt="bh0003")
    if e: return None, f"RPC: {e}"
    h = r.get("block_height")
    bc = blockcount()
...
def t98():
    sigs = []
    for i in range(5):
        r, e = roll(1,1,100,False,game_id=f"sig_chain_{i}",salt=f"sc{i:08x}")
        if e: return None, f"RPC: {e}"
...
def t117():
    hand = []
    for rd in range(10):
        r, e = roll(5,1,52,True,exclude=hand,game_id=f"card_game_{rd}",salt=f"cg{rd:04x}")
...
def t118():
    ...
    r, e = roll(5,1,52,True,exclude=excl,game_id="near_depleted",salt="cg_dep")
    if e: return None, f"RPC: {e}"
    v = r["results"]
...
def t119():
    ...
    violations = []
    for i in range(50):
        r, e = roll(1,1,100,False,exclude=excl,game_id=f"extreme_{i}",salt=f"ex{i:04x}")
        if e: continue
...
def t120():
    errors = []
    for i in range(10):
        r, e = roll(3,1,100,True,game_id=f"clean_{i}",salt=f"cl{i:04x}")
        if e: errors.append(f"{i}: {e}")
...
def t123():
    sigs = []
    for i in range(5):
        r, e = roll(1,1,100,False,game_id=f"bls_consec_{i}",salt=f"b2c{i:04x}")
...
def t124():
    ...
    beacon_expected = hashlib.sha256(bytes.fromhex(sig)).hexdigest()
    beacon_got = r.get("beacon") or r.get("round_beacon","")
    return ok(beacon_got == beacon_expected,
              f"beacon mismatch: got {beacon_got[:16]}... want {beacon_expected[:16]}...")

def t125():
    """beacon field present and 64-char hex across 5 rolls."""
    for i in range(5):
        r, e = roll(1,1,100,False,game_id=f"beacon_hex_{i}",salt=f"bh{i:04x}")
        if e: return None, f"RPC at i={i}: {e}"
        beacon = r.get("beacon") or r.get("round_beacon","")
...
def t129():
    sigs = []
    for i in range(10):
        r, e = roll(1,1,100,False,game_id=f"bls_10_{i}",salt=f"b10{i:04x}")
        if e: return None, f"RPC: {e}"
        sig = r.get("quorum_sig","")
        if len(sig)!=192: return False, f"roll {i}: sig len={len(sig)}"
        sigs.append(sig)
...
def t130():
    """beacon=SHA256(quorum_sig bytes) for 3 consecutive rolls."""
    mismatches = []
    for i in range(3):
        r, e = roll(1,1,100,False,game_id=f"sha_verify_{i}",salt=f"sv{i:04x}")
        if e: return None, f"RPC at i={i}: {e}"
        sig = r.get("quorum_sig","")
        beacon = r.get("beacon") or r.get("round_beacon","")
        if not sig or len(sig)!=192: return None, f"i={i}: sig missing or wrong length"
        expected = hashlib.sha256(bytes.fromhex(sig)).hexdigest()
        if beacon != expected:
            mismatches.append(f"i={i}: got {str(beacon)[:16]}... want {expected[:16]}...")
    if mismatches: return False, f"beacon/SHA256 mismatch: {mismatches}"
...
def t134():
    ...
    if not pm1: return None, "ptx_pose_status unavailable"
    for i in range(3):
        r, e = roll(1,1,100,False,game_id=f"ticket_roll_{i}",salt=f"tk{i:04x}")
...
def t136():
    ...
    r, e = roll(1,1,100,False,game_id="pose_decay",salt="p2d0001")
    if e: return None, f"roll failed: {e}"
    pm2 = get_pose_map()
    if not pm2: return None, "ptx_pose_status unavailable (after)"
    score_after = pm2.get("gm11",{}).get("pose_score",0)
...
def t137():
    ...
    if not pm1: return None, "ptx_pose_status unavailable"
    for i in range(3):
        r, e = roll(1,1,100,False,game_id=f"honest_{i}",salt=f"ho{i:04x}")
        if e: return None, f"roll {i} failed: {e}"
    pm2 = get_pose_map()
    if not pm2: return None, "ptx_pose_status unavailable (after)"
...
def t146():
    ...
    r, e = roll(1,1,100,False,game_id="pool_grow",salt="pg0001")
    if e: return None, f"roll failed: {e}"
...
def t148():
    ...
    bal1 = st1.get("pool_balance_sat",0)
    for i in range(3):
        r, e = roll(1,1,100,False,game_id=f"pool_3roll_{i}",salt=f"p3{i:04x}")
        if e: return None, f"roll {i} failed: {e}"
    st2, e = lottery_status()
    if e: return None, f"lottery_status post: {e}"
    bal2 = st2.get("pool_balance_sat",0)
...
def t151():
    ...
    r1, e = roll(5, 1, 52, True, game_id="chain_src", salt="txc00001")
    ...
    r2, e2 = roll(5, 1, 52, True, exclude=[tx_id], game_id="chain_dst", salt="txc00002")
    alive = node_alive()
...
def t152():
    ...
    r, e = roll(1, 1, 500, False, exclude=exclude, game_id="txid_scale", salt="ts0001")
    alive = node_alive()
...
def t154():
    """513-item exclude — one over MAX_EXCLUDE_COUNT. Must return error 1016 (EXCLUDE_LIMIT_EXCEEDED).
    Node must survive (no crash)."""
    excl = list(range(1, MAX_EXCLUDE_COUNT + 2))  # 513 items
    r, e = roll(1, 1, 1200, False, exclude=excl, game_id="excl_513", salt="e513a001")
    alive = node_alive()
    if not alive: return False, "NODE CRASHED at 513-item exclude — must return error 1016"
    if r is not None:
        return False, f"513-item exclude was accepted — should have been rejected (error 1016): {r}"
    # Check error code if it's a dict
    if isinstance(e, dict):
        code = e.get("code")
        return ok(code == 1016, f"got error code {code}, want 1016: {e}")
    # String error — at minimum the request was rejected
    return ok(True, f"rejected (string error): {str(e)[:80]}")
...
def t156():
    ...
    r, e = roll(1, 1, 100, False, exclude=[200, 300], game_id="excl_oor", salt="eo0001")
    alive = node_alive()
...
def t157():
    ...
    r, e = roll(5, 1, 6, True, exclude=[1,2,3], game_id="excl_exhaust", salt="ex0001")
    alive = node_alive()
...
def t158():
    ...
    r, e = roll(4, 1, 5, True, exclude=[2,3,4,5], game_id="excl_rt_exhaust", salt="ex0002")
    alive = node_alive()
...
def t159():
    ...
    r, e = roll(3, 1, 10, True, exclude=list(range(1,8)), game_id="excl_tight", salt="et0001")
    alive = node_alive()
...
def t160():
    ...
    r, e = roll(1, 1, 10, False, exclude=list(range(1,10)), game_id="excl_one_left", salt="eo0002")
    alive = node_alive()
...
def t161():
    ...
    r, e = roll(5, 1, 52, True, game_id="dev_seed_test", salt="ds0001", dev_seed=dev_seed_val)
    alive = node_alive()
...
def t162():
    ...
    r1, e1 = roll(3, 1, 100, False, game_id="dev_det_a", salt="ds0002", dev_seed=dev_seed_val)
    if e1:
    ...
    r2, e2 = roll(3, 1, 100, False, game_id="dev_det_a", salt="ds0002", dev_seed=dev_seed_val)
...
def t163():
    """dev_seed: different seed → different result."""
    r1, e1 = roll(3, 1, 100, False, game_id="dev_diff_a", salt="ds0003", dev_seed="aaaaaaaa"*8)
    if e1:
        if isinstance(e1,dict) and e1.get("code")==1013:
            return None, "dev_seed not available on this network"
        return None, f"first call failed: {str(e1)[:60]}"
    r2, e2 = roll(3, 1, 100, False, game_id="dev_diff_b", salt="ds0004", dev_seed="bbbbbbbb"*8)
    if e2: return None, f"second call failed: {str(e2)[:60]}"
    return ok(r1["results"]!=r2["results"],
...
def t164():
    ...
    r, e = roll(1, 1, 2000, False, exclude=excl, game_id="err_1016", salt="ec0001")
    alive = node_alive()
    if not alive: return False, "NODE CRASHED — must return error 1016"
    if r is not None: return False, f"accepted 513-item exclude — should be error 1016"
    if isinstance(e, dict):
        code = e.get("code")
        msg = e.get("message","")
...
def t166():
    ...
    long_game_id = "g" * 128
    r, e = roll(1, 1, 100, False, game_id=long_game_id, salt="gi0001")
    alive = node_alive()
...
def t167():
    ...
    r, e = roll(1, 1, 100, False, game_id="game:session_01-round.1", salt="gi0002")
    alive = node_alive()
...
def t168():
    ...
    r1, e = roll(1, 1, 100, False, game_id="shared_game_id", salt="gi0003")
    if e: return None, f"first roll: {e}"
    r2, e = roll(1, 1, 100, False, game_id="shared_game_id", salt="gi0004")
...
def t171():
    ...
    fake_txid = "f"*64
    r, e = roll(1, 1, 100, False, game_id="prev_notfound", salt="pr0001",
...
def t172():
    ...
    r1, e = roll(5, 1, 52, True, game_id="prev_unconf_src", salt="pr0002")
    if e: return None, f"source roll failed: {e}"
    ...
    r2, e2 = roll(5, 1, 52, True, game_id="prev_unconf_dst", salt="pr0003",
...
def t173():
    ...
    SKIP if prev_round_txid not yet implemented."""
    r1, e = roll(5, 1, 52, True, game_id="session_A", salt="pr0004")
    ...
    r2, e2 = roll(5, 1, 52, True, game_id="session_B", salt="pr0005",
...
def t174():
    ...
    SKIP if prev_round_txid not yet implemented."""
    r1, e = roll(5, 1, 52, True, game_id="chain_r1", salt="pr0006")
    ...
    for _ in range(20):
        time.sleep(3)
        if blockcount() > h0: break
    r2, e2 = roll(5, 1, 52, True, game_id="chain_r2", salt="pr0007",
                  prev_round_txid=tx_id)
    alive = node_alive()
...
def t175():
    ...
    for rnd in range(3):
        ...
        r, e = roll(5, 1, 52, True, game_id=f"chain3_r{rnd}", salt=f"p3r{rnd:04x}", **kwargs)
        if e:
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Hemis PTX Phase 2 Test Suite v6")
    ...
    print("═"*68)
    print(f"  HEMIS PTX PHASE 2 — LIVE NODE TEST SUITE v6  (175 tests)")
    print(f"  RPC:  {RPC_URL}")
