...
# One-shot reindex (BUG-037 recovery): if the operator dropped a REINDEX_ONCE
# marker in the datadir, reindex on THIS start only — the marker is consumed
# before launch so a restart loop cannot reindex twice.  Conf-based because
# this file regenerates Hemis.conf on every start.
...
# Ports MUST match the ptxbea chain defaults: RPC 29995 (chainparamsbase.cpp),
# P2P 29994 (chainparams.cpp). The signing fan-out derives the member RPC port
# from the chain default (PTX_FanoutRpcPort), so a mismatch here breaks signing.
exec Hemisd \
    -ptxbea \
    -datadir="$DATADIR" \
    -dnsseed=0 \
